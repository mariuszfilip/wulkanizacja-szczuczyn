...
//<![CDATA[
	...
	function real_add_page(id){
		id_added = 0;
		$('scripter').update(' ');
		new Ajax.Request('ajax.php?mod=page&act=drag_drop_add',{
	        parameters:'drag_drop=1&id='+id+'&name='+escape(jQuery('#new_page_name').val()),
			onComplete:function(response) {
				if(isUnsignedInteger(response.responseText)){
					id_added=response.responseText;
				...
				else $('scripter').update(response.responseText);				
            }
        });
	}
	...
	function real_rename_page(){
		...
		new Ajax.Request('ajax.php?mod=page&act=drag_drop_rename',{
				parameters:'id='+id+'&drag_drop=1&name='+unescape(jQuery('#page_rename_input').val()),
				onComplete:function(response) {
					$('scripter').update(response.responseText);				
				}
    });
