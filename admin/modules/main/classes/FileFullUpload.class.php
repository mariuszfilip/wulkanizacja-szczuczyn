...
class FileFullUpload {
  ...
  protected $alowed_ext = null;
  

  public function __construct( $directorty, $file, $alowed_ext = null, $ext_arr = array('php','htaccess') ) {
    $this->file = $file;
    $this->directory = $directorty;
    $this->ext_arr = $ext_arr;
		$this->alowed_ext = $alowed_ext;
    ...
    if(is_uploaded_file( $_FILES[$this->file]['tmp_name'] ) && $_FILES[$this->file]['error'] == UPLOAD_ERR_OK)
    {
			...
			if(is_array($alowed_ext)){
				if( !in_array( $this->type, $alowed_ext )) {
					return false;
				}
			}
  ...
  public function setTypeArray( array $type_array ) {
  ...
  public function move() {
    if(is_array($this->alowed_ext)){
		
		if( !in_array( $this->type, $this->alowed_ext )) {
			//echo $this->type;
			return 0;
		}
	}
