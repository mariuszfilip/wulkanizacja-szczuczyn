<table class="list" style='width: 100%;' border='0' id='stronicowanie' cellspacing='0' cellpadding='0'>
  <tr class='dolna'>
    ...
    {if $submit_ascribe ne ''}
      <input style="color: red;" value="{#form_back#}" onclick='location.href="{$smarty.server.PHP_SELF}?{$mainParameterName}={$mainParameterValue}{$return}"' type="button">
      <input class="form-button" style="color: green;" type="submit" name="{$submit_ascribe}" value="{#form_save_button#}">
    {/if}
    </td>
  </tr>
  <tr class='dolna' style='text-align:center;'>
    <td>
      Wyświetla: od <span style='font-weight: bold;'>{$page.from}</span> do
      <span style='font-weight: bold;'>{$page.to}</span> z
      <span style='font-weight: bold;'>{$page.all}</span> Ogłoszeń
    </td>
  </tr>
  </table>